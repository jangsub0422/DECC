...
Its overall structure is motivated by protein expression and SDS-PAGE-style verification.

In practical terms, the project treats:

- a natural-language specification as the upstream input,
- plasmid-style JSON blueprints as structured design artifacts,
- module-by-module code generation as expression,
- syntax and contract checks as validation gates, and
...
This is still an alpha release. Generated code may be incomplete, inconsistent, or unsuitable for production use without careful review.
...
- `code_pipeline/`: contracts, assay, protocol, connectors, and run-path helpers.
...
- An API key only if you intentionally use a hosted API path.
...
- the local Streamlit wrapper, and
- a local Ollama service for local-model workflows.
...
2. Enter a run name or project name.
...
- Some entry-module prompts may still need refinement.
...
- The pipeline is experimental and should not yet be treated as production-ready.
...
DECC는 자연어 프로그램 요구사항을 구조화된 모듈 명세로 분해하고, 각 명세를 바탕으로 Python 모듈을 생성한 뒤, 검증과 통합 단계를 거쳐 실행 가능한 결과물로 연결하는 로컬 우선 AI 코드 생성 파이프라인입니다.

이 프로젝트는 생물학적 단백질 발현 과정과 SDS-PAGE 기반 검증 흐름을 모티브로 설계되었습니다.

조금 더 구체적으로 말하면, DECC는 다음과 같은 대응 관계를 염두에 두고 설계되었습니다.

- 자연어 요구사항은 upstream 입력에 해당합니다.
- plasmid 형태의 JSON 명세는 구조화된 설계 산출물에 해당합니다.
- 모듈 단위 코드 생성은 발현 과정에 해당합니다.
- 문법 검사와 contract 검사는 검증 관문에 해당합니다.
- 최종 실행 가능한 프로그램으로의 연결은 최종 조립 단계에 해당합니다.
...
현재 DECC는 다음 profile에 대한 정규화 규칙을 포함합니다.

- `cli_crud`
- `file_transformer`
- `data_analysis`

현재는 알파 단계이므로 생성 결과가 불완전하거나 일관되지 않을 수 있으며, 충분한 검토 없이 바로 실사용하는 것은 권장하지 않습니다.
...
- `plasmids_batch/runs/`: 생성된 plasmid run 기록입니다.
- `outputs/runs/`: 생성된 Python 결과 run 기록입니다.
- `code_pipeline/`: contracts, assay, protocol, connectors, run-path helpers를 담고 있습니다.
- `web_app.py`: Streamlit 기반 wrapper UI입니다.

### 요구 사항

- Python 3.11 이상을 권장합니다.
- `requirements.txt`에 포함된 Python 패키지가 필요합니다.
- 권장 로컬 워크플로우를 위해 Ollama가 설치되어 있고 실행 중이어야 합니다.
- 상용 API 경로를 사용할 경우에만 선택적으로 API 키가 필요합니다.
...
Gemini, OpenAI, Claude 같은 상용 API는 architect 분해 단계와 모듈 코드 생성 단계 모두에서 사용할 수 있지만, 선택 사항이며 사용량 비용이 발생할 수 있습니다.
...
1. 프로그램 요구사항을 입력합니다.
2. Run name 또는 project name을 입력합니다.
3. Architect host와 model을 선택합니다.
4. plasmid를 생성합니다.
5. Current Plasmids를 검토합니다.
6. run_batch를 실행합니다.
7. integration을 실행합니다.
8. `final_app.py`를 테스트합니다.
